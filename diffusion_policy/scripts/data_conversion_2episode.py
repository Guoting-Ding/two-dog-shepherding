...
def main(config, rotate):
    ...
    for dir in dir_list:
        ...
        pos_list = np.loadtxt(path + "/posA.csv", delimiter=',', skiprows=0)
        ...
        if rotate.lower() == 'true':
            ...
            add_rotations(pos_list=pos_list,
                          sheep_pos=sheep_pos, replay_buff=replay_buffer)
...
def add_rotations( pos_list, sheep_pos, replay_buff):
    for r in range(1, 4):
        ...
        #rotated_dog_pos = [rotate_pos(dog_pos, r) for dog_pos in pos_list[:, 9:11]]
        ...
        episode = {
            ...
            #"dog_pos": np.array(rotated_dog_pos),
        }
